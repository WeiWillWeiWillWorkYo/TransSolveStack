"""Evaluate CSR selector model artifacts against runtime quality gates."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--baseline-summary",
        default="runs/phase1_csr_learning_readiness/csr_learning_summary.json",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--ranker-summary",
        default="runs/phase1_csr_linear_ranker/csr_linear_ranker_summary.json",
    )
    parser.add_argument(
        "--ranker-predictions",
        default="runs/phase1_csr_linear_ranker/csr_linear_ranker_predictions.jsonl",
    )
    parser.add_argument("--min-eval-oracle-requests", type=int, default=10)
    parser.add_argument("--min-runtime-oracle-top1-accuracy", type=float, default=0.5)
    parser.add_argument("--min-runtime-profiled-success-rate", type=float, default=2.0 / 3.0)
    parser.add_argument("--out", default="runs/phase1_csr_selector_model_eval")
    ...
    paths = {
        "rows": output / "csr_selector_model_eval_rows.jsonl",
        "summary": output / "csr_selector_model_eval_summary.json",
        "schema": output / "csr_selector_model_eval_schema.json",
        "report": output / "csr_selector_model_eval_report.md",
    ...
    write_manifest(
        build_artifact_manifest(
            artifact_kind="csr_selector_model_quality_gate",
            command="scripts/tss_csr_selector_model_eval.py",
            tracked_files=CORE_CSR_SELECTOR_MODEL_EVAL_PROVENANCE_FILES,
            metadata={
                ...
                "num_models": export.summary.num_models,

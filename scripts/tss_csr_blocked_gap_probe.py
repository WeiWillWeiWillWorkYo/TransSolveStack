"""Run blocked CSR candidate-gap CPU-screen integration probe."""
...
from transsolvestack.profiling.csr_blocked_gap_probe import (  # noqa: E402
    run_csr_blocked_gap_probe_from_files,
)
...
def main() -> None:
    ...
    parser.add_argument("--out", default="runs/phase1_csr_blocked_gap_probe")
    parser.add_argument("--device-memory-gb", type=float, default=0.20)
    parser.add_argument("--max-general-matrices", type=int, default=7)
    parser.add_argument("--max-symmetric-matrices", type=int, default=1)
    ...
    export = run_csr_blocked_gap_probe_from_files(
        matrix_queue_path=args.matrix_queue,
        coverage_summary_path=args.coverage_summary,
        output_dir=args.out,
        max_general_matrices=args.max_general_matrices,
        max_symmetric_matrices=args.max_symmetric_matrices,
    ...
    write_manifest(
        build_artifact_manifest(
            artifact_kind="csr_blocked_gap_probe",
            command="scripts/tss_csr_blocked_gap_probe.py",
            tracked_files=CORE_CSR_BLOCKED_GAP_PROBE_PROVENANCE_FILES,
            metadata={
                ...
                "newly_cpu_screen_integrated_gap_count": (
                    summary["newly_cpu_screen_integrated_gap_count"]
                ),

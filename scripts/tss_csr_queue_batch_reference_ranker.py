"""Train a shadow CSR Transformer ranker on the queue-batch training bundle."""
...
from transsolvestack.policies.csr_transformer_ranker import (  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument("--out", default="runs/phase1_csr_queue_batch_reference_ranker")
    ...
    paths = {
        "model": output / "csr_queue_batch_reference_ranker_model.json",
        "predictions": output / "csr_queue_batch_reference_ranker_predictions.jsonl",
        "summary": output / "csr_queue_batch_reference_ranker_summary.json",
        "schema": output / "csr_queue_batch_reference_ranker_schema.json",
        "report": output / "csr_queue_batch_reference_ranker_report.md",
    ...
    write_manifest(
        build_artifact_manifest(
            artifact_kind="csr_queue_batch_reference_ranker",
            command="scripts/tss_csr_queue_batch_reference_ranker.py",
            tracked_files=CORE_CSR_QUEUE_BATCH_REFERENCE_RANKER_PROVENANCE_FILES,
            metadata={
    ...
    print(f"eval_oracle_top1_accuracy: {export.summary.eval_oracle_top1_accuracy}")
    print(
        "eval_profiled_success_selection_rate: "
        f"{export.summary.eval_profiled_success_selection_rate}"
    )

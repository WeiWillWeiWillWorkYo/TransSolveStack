...
def test_taichi_csr_richardson_artifacts_are_valid():
    root = Path("runs/phase1_taichi_csr_richardson")
    rows = read_jsonl(root / "csr_richardson_results.jsonl")
    summary = json.loads((root / "csr_richardson_summary.json").read_text(encoding="utf-8"))
    manifest = read_manifest(root / "artifact_manifest.json")

    assert manifest.artifact_kind == "taichi_csr_richardson_smoke"
    ...
    assert len(rows) == 1
    assert {row["solver"] for row in rows} == {"richardson"}

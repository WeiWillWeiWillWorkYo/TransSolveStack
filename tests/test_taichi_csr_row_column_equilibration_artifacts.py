...
def test_taichi_csr_row_column_equilibration_artifacts_are_valid():
    root = Path("runs/phase1_taichi_csr_row_column_equilibration")
    rows = read_jsonl(root / "csr_row_column_equilibration_results.jsonl")
    summary = json.loads(
        (root / "csr_row_column_equilibration_summary.json").read_text(
            encoding="utf-8"
        )
    )
    schema = json.loads(
        (root / "csr_row_column_equilibration_schema.json").read_text(
            encoding="utf-8"
        )
    )
    manifest = read_manifest(root / "artifact_manifest.json")

    assert manifest.artifact_kind == "taichi_csr_row_column_equilibration_smoke"
    assert verify_manifest_hashes(manifest) == ()
    assert summary["status"] == "passed"
    assert summary["schema_version"] == "phase1_taichi_csr_row_column_equilibration_v1"
    ...
    assert "suitesparse:Zitney/extr1b" in {row["matrix_id"] for row in rows}

    for row in rows:
        assert row["backend"] == "taichi_gpu"
        assert row["gpu_executed"] is True
        assert row["preconditioner"] == "row_column_equilibration"
        assert row["trace_metadata"]["equilibration"] == "row_column_l1"

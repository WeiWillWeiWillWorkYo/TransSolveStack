...
def test_taichi_csr_bicgstab_artifacts_are_valid():
    root = Path("runs/phase1_taichi_csr_bicgstab")
    rows = read_jsonl(root / "csr_bicgstab_results.jsonl")
    summary = json.loads((root / "csr_bicgstab_summary.json").read_text(encoding="utf-8"))
    manifest = read_manifest(root / "artifact_manifest.json")

    assert manifest.artifact_kind == "taichi_csr_bicgstab_smoke"
    ...
    assert len(rows) == 4
    assert {row["solver"] for row in rows} == {"bicgstab"}
